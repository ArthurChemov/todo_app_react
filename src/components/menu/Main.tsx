...
import UncompletedTasks from "../routes/UncompletedTasks";
...
function Main (
    ...
    const [importantSide, setImportantSide] = useState<boolean>(false)
    const [completeSide, setCompleteSide] = useState<boolean>(false)
    const [uncompleteSide, setUncompleteSide] = useState<boolean>(false)
    ...
    return (
        <Flex className=' h-full w-full flex-row relative'>
            <Box className={` md:w-52 lg:w-60 md:visible fixed ${props.sidebar ? 'w-2/6 visible':'w-0 invisible'}`} >
                <Sidebar side={side} importantSide={importantSide} completeSide={completeSide} uncompleteSide={uncompleteSide} setSide={setSide} setImportantSide={setImportantSide} setCompleteSide={setCompleteSide} setUncompleteSide={setUncompleteSide}/>
            </Box>
            <Flex className={` w-full flex-col md:ml-[208px] lg:ml-[240px] ${props.sidebar ? 'small:ml-[110px]':'small:ml-0'}`}>
                ...
                <Flex className=" mt-3 flex-row justify-between">
                    ...
                    <Flex className=" mr-3">
                        ...
                        onChange={(value) => handleSorted(value.target.value)}>
                ...
                <Box className=" mx-3">
                    {side ? (
                        ...
                        {importantSide ? (
                            <>
                            <ImportantTasks grid={grid} todoList={props.todoList} setTodoList={props.setTodoList} deleteTask={deleteTask} isCompleted={isCompleted} isImportant={isImportant}/>
                            </>) : null}
                        {completeSide ? (
                            <>
                            <CompletedTasks grid={grid} todoList={props.todoList} setTodoList={props.setTodoList} deleteTask={deleteTask} isCompleted={isCompleted} isImportant={isImportant}/>
                            </>) : null}
                        {uncompleteSide ? (
                            <>
                            <UncompletedTasks grid={grid} todoList={props.todoList} setTodoList={props.setTodoList} deleteTask={deleteTask} isCompleted={isCompleted} isImportant={isImportant}/>
                            </>) : null}
                        </>
                    ) : (
                        <>
